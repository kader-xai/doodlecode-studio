 ...
 *
 * We do NOT support full CommonMark (no images, links, tables, ordered
 * lists, fenced code). That's intentional — v1 grew a giant markdown
 * surface and most of it went unused. Add only what users ask for.
 */
...
function renderInline(s: string): (string | JSX.Element)[] {
  ...
  // One regex with three alternatives. The `bold` arm must come before
  // `italic` so `**x**` isn't first parsed as `*` + `*x*` + `*`.
  const re = /(\*\*[^*]+\*\*|\*[^*\n]+\*|`[^`]+`)/g;
  let last = 0;
